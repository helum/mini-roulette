...
void main() {
  ...
  group('LocalRoulettesApi', () {
    ...
    test(
      'watchRoulettes emits the sample lunch roulette when storage is empty',
      () async {
    ...
    test(
      'saved categories are still available after a new instance is created',
      () async {
    ...
    test(
      'deleteRoulette removes the category from persisted storage',
      () async {
        final writer = await LocalRoulettesApi.create(plugin: prefs);
    ...
    test('legacy JSON without a notification key loads as disabled', () async {
      await prefs.setString(
        LocalRoulettesApi.kRoulettesStorageKey,
        '[{"id":"legacy","name":"旧データ","items":[{"id":"a","label":"ラーメン","colorValue":33133050,"weight":1}]}]',
      );

      final api = await LocalRoulettesApi.create(plugin: prefs);
      final loaded = await api.watchRoulettes().first;
      expect(loaded.single.id, 'legacy');
      expect(loaded.single.notification.enabled, isFalse);
      api.close();
    });
  });
}
